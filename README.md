...
windows. The Linux application now integrates offline BirdNET v2.4 classification;
device inference has run successfully, while labeled replay validation is the
current milestone.
...
| Offline BirdNET v2.4 adapter on the Qualcomm MPU | Device-tested, including an offline rerun; labeled replay pending |
...
| Document | Purpose |
|---|---|
| [Project story and evolution](docs/PROJECT_EVOLUTION.md) | Single narrative of the project journey, direction changes, current evidence and possible developments |
...
- `app_audio_test/`: active mono event pipeline. The folder/app name remains
  `audio-test` to match the board deployment.
...
The next demonstrable milestone is a labeled bird-call replay classified locally.
Then come the observation timeline, model evaluation on representative recordings,
and power measurements. Additional microphones, camera orientation, environmental
